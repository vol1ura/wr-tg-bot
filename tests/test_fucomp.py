...
from utils.fucomp import bot_compare, best_answer
...
def test_compare(phrase_set):
    compare = list(map(lambda p: bot_compare(p, phrase_set), test_phrases))
    assert sum(1 for b in compare if b) == 1


def test_compare_no_accost():
    compare = bot_compare('без обращения к боту', fucomp.phrases_social)
    assert not compare


def test_best_answer():
    answer = best_answer('бот, знаешь девиз?', fucomp.message_base_wr)
    print(answer)
    assert isinstance(answer, str)
    assert 'девиз' in answer
    answer = best_answer('Бот, какие кроссовки лучше для кроссов?', fucomp.message_base_m)
    print(answer)
    assert isinstance(answer, str)
